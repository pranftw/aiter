...
export class CommandProcessor {
  ...
  /**
   * Ensure the processor is initialized for the given agent with commands
   */
  private async ensureInitialized(agent: string, agentCommands: Record<string, any> = {}): Promise<void> {
    // If agent changed or not initialized, reinitialize
    if (this.agent !== agent || !this.registry) {
      this.agent = agent;
      this.registry = new CommandRegistry();
      this.initPromise = this.registry.initialize(agentCommands);
    }
  ...
  async execute(
    ...
    await this.ensureInitialized(agent, agentCommands);
   ...
   */
  async getCommands(agent: string, agentCommands: Record<string, any> = {}) {
    await this.ensureInitialized(agent, agentCommands);
   ...
   */
  async findCommands(query: string, agent: string, agentCommands: Record<string, any> = {}) {
    await this.ensureInitialized(agent, agentCommands);
