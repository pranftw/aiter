...
const editSnippetToolInputSchema = z.object({
...
const editSnippetToolOutputSchema = z.object({
  response: z.string(),
});


export const edit_snippet = tool({
  name: 'edit_snippet',
  ...
  inputSchema: editSnippetToolInputSchema,
  outputSchema: editSnippetToolOutputSchema,
  execute: async ({id, description, strToReplace, strToReplaceWith}, {experimental_context: context}) => {
    ...
    const {result: {chat, snippet}, status: getSnippetStatus, message: getSnippetMessage} = await getSnippet(chatId, id);
    ...
    if (snippetContent.includes(strToReplace)) {
      chat.snippets[id].content = snippetContent.replace(strToReplace, strToReplaceWith);
      const lintResult = lintPython('snippet.py', chat.snippets[id].content);
      if (lintResult) {
        return {
          response: lintResult,
        };
    ...
    if (description) {
      chat.snippets[id].description = description;
    }
